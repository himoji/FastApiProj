...
class atm():
    def deposit(customer_id: int, cash: int) -> bool:
        '''
    ...
    def send(customer_id, cash, taker_id) -> bool:
        '''
    Send command to update CASH at customer_id and taker_id.\n
    ...
    '''
        ...
        try:
            ...
            if cash >= 0 and cash_on_sender_card - cash >= 0 and securityCheck(customer_id, taker_id)==False:
                print("Sending cash")
                cursor.execute(f"update customers_table set customer_cash = customer_cash + {cash} where customer_name = '{taker_id}';")
                print("send #1")
                cursor.execute(f"update customers_table set customer_cash = customer_cash - {cash} where customer_name = '{customer_id}';")
                print("send #2")
                cursor.close()
                db.commit()
                return True
    ...
    def getCashAmountByName(customer_name) -> int:
        cursor = db.cursor()
    ...
    def getCashAmountById(customer_id) -> int:
        cursor = db.cursor()
        redis_client = redis.Redis("localhost", 6379)
